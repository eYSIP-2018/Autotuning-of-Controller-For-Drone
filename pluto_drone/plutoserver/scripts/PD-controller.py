...
class drone_control():
	def __init__(self):
		...
		# Kp, Kd values of Pitch, Roll, Throttle
		self.pitch_kp = 27.78
	...
	def pid(self):
		...
		# else ; compute pid output
			...
			self.out_roll = (self.roll_kp * error_y)  +  (self.roll_kd * (error_y- self.prev_error_y))
			self.out_pitch = (self.pitch_kp * error_x) + (self.pitch_kd *(error_x - self.prev_error_x))
			self.out_throttle = (self.throttle_kp * error_z)  + (self.throttle_kd * (error_z - self.prev_error_z))
